...
theGame.prototype = {
	create : function() {
		...
		//Text
		timeLastText = this.game.add.text(512, 650, "Time Left : " , { font: '30px Courrier New', fill: '#ffaa00' });
		timeLastText.anchor.setTo(0.5,0);
		weaponText = this.game.add.text(512,680, "Your weapon : " , { font: '30px Courrier New', fill: '#ffaa00' });
		weaponText.anchor.setTo(0.5,0);
		scoreText = this.game.add.text(25, 50, "Score : " , { font: '30px Courrier New', fill: '#ffaa00' });
		waveText = this.game.add.text(25, 25, "Wave : " , { font: '30px Courrier New', fill: '#ffaa00' });
		//Buttons
		if(song.paused) {
		...
		bossBullets.createMultiple(30, 'shootBoss', 0, false);
		//Explosions
		explosions = game.add.group();
		for (var i = 0; i < 10; i++)
		{
			var explosionAnimation = explosions.create(0, 0, 'kaboum', [0], false);
			explosionAnimation.anchor.setTo(0.5, 0.5);
			explosionAnimation.animations.add('kaboom');
		}
		//Sounds
		explosionSound = this.game.add.sound('explosion',0.15);
		shootSound = this.game.add.sound('shoot',0.1);
		shootBossSound = this.game.add.sound('shootBossSound',0.15);
		//Luncher
		this.spawnPlayer();
		this.FnextWave();
	},
	update : function() {
		...
		this.game.physics.arcade.collide(bads,bads);
		if(bossBool) {
			if(!invincible) {
		...
		//Invincible
		if(invincibleTime < this.game.time.now && invincible) {
			invincible = false;
		}
		if(invincible) {
			player.animations.play("invincible");
		}
		else {
	...
	spawnPlayer : function() {
		...
		player.animations.add('invincible', [1, 0],10,true);
	...
	killBad : function(bullet, bad) {
		...
		explosionAnimation.scale.setTo(2,2);
	...
	killPlayer : function(player, bad) {
		...
		explosionSound.play();
		invincible = true;
		invincibleTime = this.game.time.now + 2000;
		if(life<=0) {
			this.gameOver();
		}
	},
